 ...
 * Date: May 13, 2018
 * Description: Random tester for the Village card function in dominion.c
 * */
...
int testVillageCard(struct gameState *post, int cP)
{
    ...
    r = cardEffect(village, 0, 0, 0, post, (post->handCount[cP] - 1), NULL);
    ...
    /* post village card
     * handSize should be +1
     * numActions should be 2
     * Nothing else in gameState should be changed
    */

    int handSizeDiff = post->handCount[cP] - pre.handCount[cP];
    a = asserttrue(0, handSizeDiff, "The current player's hand is not the same size.");
    if (a == 0)
        flag = 1;

    int numActionsDiff = post->numActions - pre.numActions;
    a = asserttrue(2, numActionsDiff, "The number of actions was not increased by 2.");
    if (a == 0)
        flag = 1;
...
int main() {
    ...
    for (int n = 0; n < 2000; n++) {
        ...
        t = testVillageCard(&G, currentPlayer);
        if (t == 1)
            didATestFail = 1;
    }
    if (didATestFail == 0) 
