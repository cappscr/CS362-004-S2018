...
{
  /*
  printf("Current player deck count (INSIDE SHUFFLE): %d\n", state->deckCount[player]);
  printf("Current player discard count (INSIDE SHUFFLE): %d\n", state->discardCount[player]);
  printf("Current player hand count (INSIDE SHUFFLE): %d\n", state->handCount[player]); 
  */
  ...
  //printf("Made it past the qsort call\n");
  ...
  for (i = 0; i < newDeckPos; i++)
  {
    ...
    state->deckCount[player]++;
    //printf("Deck count value: %d\n", state->deckCount[player]);
  }
  //printf("Next line returns\n");
  return 0;
}
...
int adventurerCard(struct gameState *state, int *drawntreasure, int *currentPlayer, int temphand[MAX_HAND], int *z)
{
  ...
  /*
    ...
    printf("Value of drawntreasure is: %d\n", *drawntreasure);
    */
  int cardDrawn;
  int shuffleCalled = 0;
  ...
  {
    //printf("Current player deck count (TOP OF WHILE): %d\n", state->deckCount[*currentPlayer]);
    if (state->deckCount[*currentPlayer] < 1 && shuffleCalled <= 1)
    { //if the deck is empty we need to shuffle discard and add to deck
      //printf("Made it inside the shuffle card\n");
      if (shuffleCalled == 1)
        break;
      //printf("Current player deck count: %d\n", state->deckCount[*currentPlayer]);
      //printf("Current player discard size: %d\n", state->discardCount[*currentPlayer]);
      ...
      //printf("Current player deck count (AFTER MANIP): %d\n", state->deckCount[*currentPlayer]);
      //printf("Current player discard size (AFTER MANIP): %d\n", state->discardCount[*currentPlayer]);
      //printf("Current player is: %d\n", *currentPlayer);
      shuffle(*currentPlayer, state);
      shuffleCalled = 1;
      //printf("Made it past the shuffle card\n");
      //printf("Current player deck count: %d\n", state->deckCount[*currentPlayer]);
    }
    //printf("About to call draw card\n");
    drawCard(*currentPlayer, state);
    //printf("Made it past the draw card call\n");
    //printf("About to set cardDrawn\n");
    //printf("Size of hand: %d\n", state->handCount[*currentPlayer]);
    cardDrawn = state->hand[*currentPlayer][state->handCount[*currentPlayer] - 1]; //top card of hand is most recently drawn card.
    
    //printf("Past setting cardDrawn\n");
    //printf("The value of drawntreasure is: %d\n", *drawntreasure);
    //printf("The value of carddrawn is: %d\n", cardDrawn);
    
    if (cardDrawn == copper || cardDrawn == silver || cardDrawn == gold)
    {
      //printf("About to decrement drawntreasure\n");
      //printf("The value of drawntreasure is: %d\n", *drawntreasure);
      (*drawntreasure)++;
      //printf("Past decrementing drawn treasure\n");
    }
    else
    {
      //printf("about to access tempHand\n");
      //printf("The value of z is %d\n", *z);
      temphand[*z] = cardDrawn;
      //printf("past temphand access\n");
      //printf("Before decrementing handCount\n");
      state->handCount[*currentPlayer]--; //this should just remove the top card (the most recently drawn one).
      //printf("After decrementing handCount\n");
      (*z)++;
    }
  }
  while ((*z) - 1 >= 0)
  {
    //printf("Value of z: %d\n", *z);
    //printf("Before accessing discard\n");
    //printf("Discard Count before: %d\n", state->discardCount[(*currentPlayer)]);
    state->discard[*currentPlayer][state->discardCount[(*currentPlayer)]++] = temphand[(*z) - 1]; // discard all cards in play that have been drawn
    //printf("Discard Count after: %d\n", state->discardCount[(*currentPlayer)]);
    (*z)--;
    //printf("After accessing discard\n");
  }
  //printf("RETURNING FROM ADV CARD\n");
  return 0;
}
